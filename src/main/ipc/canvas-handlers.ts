...
import { isInitialized, getSpace, getSkill, assignSpaceFolder, updateCanvasContent } from '../storage';
import { getConfigValue } from '../config';
import { scheduleAutoCommit, resolveAttachmentPath, getMimeType, getSpaceHistory, restoreSpaceVersion, getSpaceVersionContent, resolveSpaceFolder, resolvePagePath } from '../workspace';
import { initSpaceCanvas, ensureSpaceCanvas, readCanvas, saveAttachment, createPage, readPage, listPages } from '../storage';
import { parseFrontmatter, serializeFrontmatter } from '../frontmatter';
import { fetchLinkPreview } from '../services/link-preview';
import { startWatching, stopWatching } from '../canvas-watcher';
import { mirrorRendererEvent } from '../web/event-hub';
import { forgetCanvasEditorContent, rememberCanvasEditorContent, writeEditorFileWithMergeAsync, writeMainCanvasWithMergeAsync, type CanvasWriteResult } from '../services/canvas-editor-state';
...
import { writeDocument, readDocument } from '../storage';

const CANVAS_FILE = 'canvas.md';

/** Space lookup for the canvas file resolver, which stays database-agnostic. */
const spaceFolderLookup = async (spaceId: string): Promise<string | null> => (await getSpace(spaceId))?.folder ?? null;

/** Decide what a link means, or null when there is no workspace to decide against. */
async function resolveLinkFor(spaceId: string, url: string): Promise<LinkTarget | null> {
  const workspace = getConfigValue('workspace');
  if (!workspace) return null;
  return resolveLinkTarget(url, {
    baseDir: (await resolveCanvasBaseDir(workspace, spaceId)),
    isWorkspaceMdFile,
  });
}
...
function watchEditorFile(editorId: string, filePath: string, onChange?: (content: string) => void): void {
  startWatching(editorId, filePath, async (content) => {
...
async function saveSkillContent(workspace: string, editorId: string, content: string): Promise<CanvasWriteResult> {
  const skill = await getSkill(editorId.slice('__skill__'.length));
  if (!skill) return { success: false, error: 'not_found' };
  const { frontmatter, body } = parseFrontmatter<SkillFrontmatter>(content);
  return writeEditorFileWithMergeAsync(editorId, skill.filePath, serializeFrontmatter(frontmatter, body), async (merged, expected) => {
...
export function registerCanvasHandlers(): void {
  ...
  registerIpcHandler('canvas:has-content', async (_event, spaceId: string) => {
    const workspace = getConfigValue('workspace');
    ...
    if (spaceId.startsWith('__file__')) {
      ...
      if (!isWorkspaceMdFile(filePath)) return { hasContent: false };
      try {
        return { hasContent: (await readDocument(filePath, workspace)).trim().length > 0 };
      } catch {
    ...
    try {
      ...
      return { hasContent: (await readDocument(canvasPath, workspace)).trim().length > 0 };
    } catch {
      return { hasContent: false };
    }
  });

  registerIpcHandler('canvas:read', async (_event, spaceId: string) => {
    const workspace = getConfigValue('workspace');
    ...
    if (spaceId.startsWith('__skill__')) {
      const skill = await getSkill(spaceId.slice('__skill__'.length));
      if (!skill) return { content: '', error: 'not_found' };
      const content = await readDocument(skill.filePath, workspace);
      rememberCanvasEditorContent(spaceId, content);
      watchEditorFile(spaceId, skill.filePath);
      return { content };
    }
    ...
    if (pageTarget) {
      ...
      const result = (await readPage(workspace, folder, pageName));
    ...
    const content = (await readCanvas(workspace, folder));
    rememberCanvasEditorContent(spaceId, content);
    ...
    watchEditorFile(spaceId, canvasPath, async (newContent: string) => {
      const titleUpdate = (await updateCanvasContent(spaceId, newContent));
  ...
  registerIpcHandler('canvas:write', async (_event, spaceId: string, content: string) => {
    const workspace = getConfigValue('workspace');
    ...
    if (spaceId.startsWith('__file__')) {
      ...
      const result = await writeEditorFileWithMergeAsync(spaceId, filePath, content, async (contentToWrite, expected) => {
        if (!isWorkspaceMdFile(filePath)) throw new Error('not_in_workspace');
        await writeDocument({ filePath, root: workspace, content: contentToWrite, expected });
      });
      if (result.success) {
        scheduleAutoCommit(workspace);
      }
    ...
    if (spaceId.startsWith('__skill__')) {
      return saveSkillContent(workspace, spaceId, content);
    }
    ...
    if (pageTarget) {
      ...
      return writeEditorFileWithMergeAsync(spaceId, resolvedPage.path, content, async (contentToWrite, expected) => {
        await writeDocument({ filePath: resolvedPage.path, root: workspace, content: contentToWrite, expected });
      });
    }
  ...
  registerIpcHandler('canvas:close', async (_event, spaceId: string, content: string) => {
    const workspace = getConfigValue('workspace');
    if (!workspace || !isInitialized()) return { success: false, error: 'no_workspace' };

    if (spaceId.startsWith('__skill__')) {
      return closeSucceeded(await saveSkillContent(workspace, spaceId, content), workspace, spaceId);
    }
    ...
    if (pageTarget) {
      ...
      const resolvedPage = resolvePagePath(workspace, folder, pageName);
      if ('error' in resolvedPage) return { success: false, error: resolvedPage.error };
      const result = await writeEditorFileWithMergeAsync(spaceId, resolvedPage.path, content, async (contentToWrite, expected) => {
        await writeDocument({ filePath: resolvedPage.path, root: workspace, content: contentToWrite, expected });
      });
      return closeSucceeded(result, workspace, spaceId);
    }
    ...
    return closeSucceeded(await writeMainCanvasWithMergeAsync(workspace, spaceId, folder, content), workspace, spaceId);
  });

  // ── Canvas file paste ─────────────────────────────────
  registerIpcHandler('canvas:paste-file', async (_event, spaceId: string, filename: string, dataArray: number[]) => {
    const workspace = getConfigValue('workspace');
    ...
    const result = (await saveAttachment(workspace, folder, filename, data));
    return result;
  });

  // ── Attachment file serving ───────────────────────────
  registerIpcHandler('canvas:resolve-attachment', async (_event, spaceId: string, relativePath: string) => {
    const workspace = getConfigValue('workspace');
    if (!workspace || !isInitialized()) return { error: 'no_workspace' };
  ...
  registerIpcHandler('canvas:read-file', async (_event, spaceId: string, relativePath: string) => {
    const workspace = getConfigValue('workspace');
    if (!workspace || !isInitialized()) return { error: 'no_workspace' };
    ...
    const folder = await spaceFolderLookup(page?.realSpaceId ?? spaceId);
    const result = readCanvasFile(workspace, spaceId, relativePath, () => folder);
  ...
  registerIpcHandler('canvas:open-folder', async (_event, spaceId: string) => {
    const workspace = getConfigValue('workspace');
  ...
  registerIpcHandler('canvas:restore', async (_event, spaceId: string, sha: string) => {
    ...
    if (result.success) {
      ...
      const titleUpdate = (await updateCanvasContent(spaceId, content));
      if (titleUpdate.titleChanged) {
        for (const win of BrowserWindow.getAllWindows()) {
          win.webContents.send('space:title-updated', { spaceId, title: titleUpdate.title });
        }
  ...
  registerIpcHandler('canvas:read-activity-log', async (_event, spaceId: string) => {
    ...
    const { readSpaceActivityLog } = await import('../storage');
    return { events: await readSpaceActivityLog(workspace, space.folder) };
  });

  // ── Child pages ──────────────────────────────────────────
  registerIpcHandler('canvas:create-page', async (_event, spaceId: string, pageName: string) => {
    const workspace = getConfigValue('workspace');
    ...
    const result = (await createPage(workspace, folder, pageName));
  ...
  registerIpcHandler('canvas:read-page', async (_event, spaceId: string, pageName: string) => {
    const workspace = getConfigValue('workspace');
    if (!workspace || !isInitialized()) return { content: '', error: 'no_workspace' };
  ...
  registerIpcHandler('canvas:write-page', async (_event, spaceId: string, pageName: string, content: string) => {
    const workspace = getConfigValue('workspace');
    if (!workspace || !isInitialized()) return { success: false, error: 'no_workspace' };
    ...
    if ('error' in resolvedPage) return { success: false, error: resolvedPage.error };
    const editorId = pageEditorId(spaceId, pageName);
    return writeEditorFileWithMergeAsync(editorId, resolvedPage.path, content, async (contentToWrite, expected) => {
      await writeDocument({ filePath: resolvedPage.path, root: workspace, content: contentToWrite, expected });
    });
  });

  registerIpcHandler('canvas:close-page', async (_event, spaceId: string, pageName: string, content: string) => {
    const workspace = getConfigValue('workspace');
    if (!workspace || !isInitialized()) return { success: false, error: 'no_workspace' };
    ...
    if ('error' in resolvedPage) return { success: false, error: resolvedPage.error };
    const editorId = pageEditorId(spaceId, pageName);
    const result = await writeEditorFileWithMergeAsync(editorId, resolvedPage.path, content, async (contentToWrite, expected) => {
      await writeDocument({ filePath: resolvedPage.path, root: workspace, content: contentToWrite, expected });
    });
    return closeSucceeded(result, workspace, editorId);
  });

  registerIpcHandler('canvas:list-pages', async (_event, spaceId: string) => {
    const workspace = getConfigValue('workspace');
    ...
    return { pages: (await listPages(workspace, folder)) };
  });
  ...
  registerIpcHandler('canvas:open-link', async (_event, spaceId: string, url: string) => {
    const target = (await resolveLinkFor(spaceId, url));
  ...
  registerIpcHandler('canvas:resolve-link', async (_event, spaceId: string, url: string) => {
    return (await resolveLinkFor(spaceId, url)) ?? { kind: 'none' as const, reason: 'no_workspace' as const };
  });
  ...
  registerIpcHandler('canvas:get-agent-state', async (_event, spaceId: string) => {
    ...
    return (await getCanvasAgentState(spaceId));
  });
}

/** Resolve the base directory for relative path resolution based on the canvas context. */
async function resolveCanvasBaseDir(workspace: string, spaceId: string): Promise<string | null> {
  if (spaceId.startsWith('__file__')) {
    const filePath = decodeURIComponent(spaceId.slice('__file__'.length));
